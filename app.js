...
async function bookAppointment(barberId, barberName) {
  ...
  try {
    ...
    await set(newBookingRef, {
      ...
      timestamp: Date.now(),
      isActive: true // إضافة حقل لتتبع حالة الحجز
    });
...
async function loadBarberQueue() {
  ...
  state.queueListeners[state.currentUser.id] = onValue(queueRef, (snapshot) => {
    ...
    // تحويل القائمة إلى مصفوفة وفرزها حسب الوقت
    const queueArray = Object.entries(queue)
      .filter(([_, value]) => value.isActive !== false)
      .map(([key, value]) => ({
        id: key,
    ...
    // عرض العملاء مع وضع الحجوزات النشطة أولاً
...
async function completeClient(barberId, bookingId) {
  ...
  try {
    // حذف الحجز نهائياً عند إكمال الخدمة
    await remove(ref(database, `barbers/${barberId}/queue/${bookingId}`));
    
    // إذا كان هذا الحجز للعميل الحالي، عرض نموذج التقييم
